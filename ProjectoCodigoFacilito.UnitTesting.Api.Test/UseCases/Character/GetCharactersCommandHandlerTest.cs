...
{
    public class GetCharactersCommandHandlerTest
    {
        [Fact]
        public async Task GetAllCharacterCommandHandler_Success()
        {
            // Arrange
            ...
            GetCharacterQueryHandler getCharacterQueryHandler = new (characterRepository, mapper);

            // Act
            var result = await getCharacterQueryHandler.Handle(new GetCharacterQuery(), new System.Threading.CancellationToken());

            // Assert
            Assert.NotNull(result);
            Assert.Single(result);
            Assert.IsType<List<CharacterDTO>>(result);
